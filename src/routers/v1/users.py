...
async def create_user(user_in: UserCreateWithOrder,
                      session: AsyncSession = Depends(get_session),
                      order_client: OrderServiceClient = Depends(get_order_client),):
    return await UserService(session, order_client).create_user(user_in)
...
async def read_user(user_id: UUID,
                    session: AsyncSession = Depends(get_session),
                    order_client: OrderServiceClient = Depends(get_order_client),):
    return await UserService(session, order_client).get_user_with_orders(user_id)
...
async def update_user(user_id: UUID,
    ...
    return await UserService(session, order_client).update_user(user_id, user_in)
...
async def delete_user(user_id: UUID,
    ...
    await UserService(session, order_client).delete_user(user_id)
